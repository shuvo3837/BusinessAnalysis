...
async function startServer() {
  ...
  const DB_FILE_PATH = path.join(process.cwd(), "store-db.json");

  // Local storage cache representation
  let db: {
    users: User[];
    products: Product[];
    sales: SaleRecord[];
  ...
  // Load actual DB file or seed if empty
  if (fs.existsSync(DB_FILE_PATH)) {
    try {
      const data = fs.readFileSync(DB_FILE_PATH, "utf-8");
      db = JSON.parse(data);
      console.log(`Database loaded successfully. ${db.products.length} products, ${db.sales.length} sales found.`);
    } catch (e) {
      console.error("Failed to read database file. Re-seeding.");
      seedDb();
    }
  } else {
    seedDb();
  ...
  app.put("/api/inventory/:id", (req, res) => {
    ...
    db.products[index] = {
      ...
      costPrice: costPrice !== undefined ? Number(costPrice) : db.products[index].costPrice,
      sellingPrice: sellingPrice !== undefined ? Number(sellingPrice) : db.products[index].sellingPrice,
      category: category || db.products[index].category,
      marketplace: marketplace || db.products[index].marketplace
    };

    writeDb();
    res.json({ product: db.products[index], message: "Inventory record updated." });
  });

  app.delete("/api/inventory/:id", (req, res) => {
    const { id } = req.params;
    db.products = db.products.filter(p => p.id !== id);
    db.sales = db.sales.filter(s => s.productId !== id); // Cascade delete matching sales
    writeDb();
  ...
  app.post("/api/sales", (req, res) => {
    ...
    linkedProduct.stock = Math.max(0, linkedProduct.stock - qty);

    db.sales.push(newSale);
    writeDb();
    res.json({ sale: newSale, message: "Sale logged and inventory adjusted." });
  });
  ...
  app.post("/api/import-csv", (req, res) => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const existingId = db.products.findIndex(p => p.sku === sku);
        if (existingId !== -1) {
          db.products[existingId].stock += stock;
          db.products[existingId].costPrice = costPrice;
          db.products[existingId].sellingPrice = sellingPrice;
        } else {
          db.products.push({
            id: "p-" + Math.random().toString(36).substr(2, 9),
            sku,
...
Note: For the 'analytics' object, generate an updated mock/simulated full analytical context relying on these new single-day/note inputs extrapolated out, so it can be updated into the system's global context seamlessly.` }
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    // Context formatting - give Gemini a complete picture of the exact inventory, low stock values, sales, and profit.
    const inventorySnapshot = db.products.map(p => `Name: ${p.name}, SKU: ${p.sku}, Stock: ${p.stock}, Selling Price: $${p.sellingPrice}, Marketplace: ${p.marketplace}`).join("\n");
    
    // Aggregated Metrics
    const totalRev = db.sales.reduce((sum, item) => sum + item.totalRevenue, 0);
    const totalProf = db.sales.reduce((sum, item) => sum + item.profit, 0);
    const topProducts = db.products.map(p => {
      const salesVol = db.sales.filter(s => s.productId === p.id).reduce((sum, s) => sum + s.quantity, 0);
      return { name: p.name, volume: salesVol };
    }).sort((a, b) => b.volume - a.volume).slice(0, 3);
    const topProductsStr = topProducts.map(tp => `${tp.name} (${tp.volume} units sold)`).join(", ");

    try {
      const isKeySet = !!process.env.GEMINI_API_KEY;
      if (!isKeySet) {
        // Standard high-quality responsive partner chat response simulation without key
        let reply = "";
        const lower = message.toLowerCase();
        if (customContext) {
           reply = `I'm analyzing your uploaded data! Based on the insights, your business category is **${customContext.businessCategory}**. You have a total revenue of $${customContext.performance?.totalRevenue} with a gross profit margin of ${customContext.performance?.grossProfitMargin}%. \n\nTarget customers are primarily ${customContext.persona?.ageGroup} ${customContext.persona?.gender}. \n\nI recommend: ${customContext.strategies?.[0]}.`;
        } else if (lower.includes("restock") || lower.includes("stock") || lower.includes("inventory")) {
          const critical = db.products.filter(p => p.stock <= 15);
          reply = `Let's review your inventory! You currently have **${critical.length} product(s)** needing urgent attention: \n\n` + 
            critical.map(c => `- **${c.name}** has only **${c.stock} items** left on **${c.marketplace}**.\n`).join("") + 
            `\nI recommend boosting stock for fast-sellers to avoid running dry. Let me know if you would like automated campaign recommendations for any of these!`;
        } else if (lower.includes("sales") || lower.includes("revenue") || lower.includes("profit")) {
          reply = `According to active records, your overall shop revenue stands at **$${totalRev.toFixed(2)}** with a net profit margin of **$${totalProf.toFixed(2)}**.\n\nYour best performing catalogs are: **${topProductsStr}**.\n\nDo you want me to plan an ad budget allocation or forecast the next 30 days of performance?`;
        } else {
          reply = `I have analyzed your e-commerce data! We currently have ${db.products.length} catalog streams running. Your total logged profit is $${totalProf.toFixed(2)}. \n\nYou can ask me specific tasks like:\n1. "Which products should I restock first?"\n2. "Generate an ad budget breakdown for my channels."\n3. "Optimize my classic backpack listings."\n\n(Please note: Enter your Gemini API secret key in AI Studio > Settings > Secrets to enable full power conversation logic!)`;
        }
        ...
        writeDb();
        return res.json({ chat: modelReply, isDemo: true });
      }

      // Build context prompt
      const systemInstruction = `
        You are a brilliant, elite eCommerce Business Partner & Analytics Copilot.
        You have direct real-time access to the user's business database.
        
        MUST ADAPT TO CUSTOM UPLOADED CONTEXT IF PROVIDED BELOW:
        --- CUSTOM CSV / EXCEL ANALYTICS CONTEXT ---
        ${customContext ? JSON.stringify(customContext, null, 2) : "None provided. Rely on Live Database State."}
        ...
        --- BUSINESS METRICS ---
        - Combined Channel Revenue: $${totalRev.toFixed(2)}
        - Combined Profit: $${totalProf.toFixed(2)}
        - Top Selling Catalogs: ${topProductsStr}
        
        Guidelines:
        1. If Custom CSV context exists, base your persona, marketing, and analytical responses on IT. Be deeply analytical, business-focused, professional and precise. The business category is highly important.
        2. If relying on Live DB state, quote REAL product names, exact metrics, and precise channel details from the snapshot above.
        3. Do not generate generic marketing hype. Provide clear structured responses in neat markdown with bolding. Keep answers direct, tactical, and actionable.
      `;

      // Grab chat history up to past 10 messages for context memory
      const historyContext = db.chats.slice(-10).map(c => ({
      ...
      db.chats.push(modelReply);
      writeDb();
      res.json({ chat: modelReply, isDemo: false });

    } catch (e: any) {
      console.error("Gemini Chat failed, generating custom smart reply:", e);
      const fallbackReply: ChatMessage = {
        id: "c-m-" + Math.random().toString(36).substr(2, 9),
        role: "model",
        text: `I apologize, there was an issue processing that query with the AI model. Let me address it mathematically using your direct store coordinates:\n\n- Active products: ${db.products.length}\n- Current revenue tracked: $${totalRev.toFixed(0)}\n- Current overall profit margin: $${totalProf.toFixed(0)}\n\nWhat other analytical view or manual spreadsheet operation can I run for you?`,
        createdAt: new Date().toISOString()
      };
      db.chats.push(fallbackReply);
      writeDb();
      res.json({ chat: fallbackReply, isDemo: true });
    }
  });
